...
- `scripts/install.sh`: dry-run and `--apply`/`--revert` verified for real via SSH against a private, snapshotted test instance — same "manually verified, no automated test" precedent the parent spec already establishes for the one piece that touches a live remote system.
